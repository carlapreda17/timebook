    ...
    <div ref={ref} className="flex items-center gap-4 relative pl-3">
      ...
      >
        <div className="flex items-center gap-4">
          <div className="w-9 h-9 tablet:w-7 tablet:h-7 rounded-full flex justify-center items-center bg-secondary">
            <i className="text-lg tablet:text-sm text-white fas fa-user"></i>
          </div>
          {status === 'loading' && (
            <div className="bg-gray-400 h-3 w-20 rounded-full hidden sm:block" />
          )}
          {status === 'success' && (
            <h4 className="text-gray-900 dark:text-gray-200 font-semibold text-sm hidden sm:block">
              {me.first_name} {me.last_name}
            </h4>
          )}
        </div>
        <i
          className={`fas ${isOpen ? 'fa-chevron-up' : 'fa-chevron-down'} text-gray-500 hidden sm:block`}
        ></i>
      </div>
      {isOpen && (
